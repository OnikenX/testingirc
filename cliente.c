...
int main( int argc , char *argv[] )
{
	...
	serv_addr.sin_port = htons((u_short)atoi(argv[3]));
	...
	if(sendto( s , (char *)response, strlen(response), 0 ,(struct sockaddr*)&serv_addr , sizeof(serv_addr) ) == SOCKET_ERROR)
	...
	nbytes = recvfrom(s, (char *)response, strlen(response), 0, (struct sockaddr *)&serv_addr, &size);
